...
for i in range(1,101):
    ...
    # Run the experiment ops
    session.run(
            experiment_ops,
            options = run_options,
            run_metadata = run_metadata)
    
    # THIS STUFF IS COMMENTED OUT TO SPEED UP RENDERING, BUT CAN BE USED IF DESIRED
...
with tf.variable_scope("image_rendering"):
    ...
    # Scale the values within the div_count matrix to within the desired range
    color_cycle_factor = tf.constant(
            1,
            dtype = tf.float64,
            name = "color_cycle_factor")
    div_count_scaled = tf.multiply(
            div_count_reshape,
            color_cycle_factor,
            name = "div_count_scaled")
    ...
    with tf.variable_scope("red_transform"):
        ...
        red_h_scale = tf.multiply(div_count_scaled, red_k, name = "red_h_scale")
    ...
    with tf.variable_scope("green_transform"):
        ...
        green_h_scale = tf.multiply(div_count_scaled, green_k, name = "green_h_scale")
    ...
    with tf.variable_scope("blue_transform"):
        ...
        blue_h_scale = tf.multiply(div_count_scaled, blue_k, name = "blue_h_scale")
